...
public:
    ...
    uint32_t current_handle() const noexcept { return current_handle_; }
    uint32_t target_live_handle() const noexcept;

    // Drop the installed DSP so the next retarget pass can attach to the
    // latest live RadioStreamFmod handle. Used only after callback staleness.
    void detach_current() noexcept;

    // Attach to the RadioStreamFmod handle currently stored at +0x20. Returns
    // false when there is no live handle or it is the handle we already own.
    bool retarget_to_target() noexcept;
...
private:
    ...
    std::atomic<bool> force_stereo_audio_{true};
