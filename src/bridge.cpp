...
// Keep the runtime check limited to the license notice the packaged web UI
// needs. Credits, donation links, and upstream promotion are documentation
// choices, not something the DLL should enforce at startup.
bool verify_ui_license_notice(const std::filesystem::path& ui_dir) {
    ...
    constexpr std::array<std::string_view, 2> required = {"GPLv3", "LICENSE"};
    for (auto needle : required) {
        if (html.find(needle) == std::string::npos) {
            log::error("[bridge] webui is missing license marker '{}' -- refusing to start.",
                       std::string{needle});
            return false;
        }
    }
...
void open_dashboard(uint16_t port) noexcept {
    std::wstring url = L"http://localhost:";
    url += std::to_wstring(port);
    url += L"/";

    auto rc = reinterpret_cast<INT_PTR>(
        ShellExecuteW(nullptr, L"open", url.c_str(), nullptr, nullptr, SW_SHOWNORMAL));
    if (rc <= 32) {
        log::warn("[bridge] failed to open dashboard in default browser (ShellExecute rc={})",
                  rc);
    } else {
        log::info("[bridge] opened dashboard in default browser: http://localhost:{}/", port);
    }
}
...
void run_bridge(HMODULE self) noexcept {
    ...
    if (!verify_ui_license_notice(ui_dir)) {
        log::error("[bridge] aborting startup: webui license notice check failed");
        return;
    }
    ...
    if (cfg.general.open_dashboard_on_start) {
        open_dashboard(cfg.general.port);
    } else {
        log::info("[bridge] dashboard auto-open disabled by config");
    }
