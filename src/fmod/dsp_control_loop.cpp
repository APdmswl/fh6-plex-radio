...
void ControlLoop::recover_stale_dsp() noexcept {
    ...
    bridge_.set_target(*chosen, fmod_system);
    meta_.set_target(chosen->sample_props_body);
    const auto live_handle = bridge_.target_live_handle();
    if (!live_handle) return;

    if (live_handle == bridge_.current_handle() && bridge_.current_handle_alive()) {
        if (last_stale_wait_handle_ != live_handle) {
            last_stale_wait_handle_ = live_handle;
            log::warn("[ctrl] DSP stale on still-live handle 0x{:X}; waiting for FH6 to rotate "
                      "the radio channel",
                      live_handle);
        }
        return;
    ...
    last_stale_wait_handle_ = 0;
    if (bridge_.retarget_to_target()) {
