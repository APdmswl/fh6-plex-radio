...
uint32_t DSPBridge::target_live_handle() const noexcept {
    return read_live_handle(radio_stream_);
}

void DSPBridge::detach_current() noexcept {
    release_current_dsp_locked();
}
...
void DSPBridge::retarget_if_needed() noexcept {
    ...
    // FH6 refreshes the RadioStreamFmod channel handle even while the current
    // DSP is still being mixed. Chasing every fresh handle causes mid-song
    // gain jumps, so stay pinned until the current DSP actually goes stale.
    if (current_handle_alive()) return;

    (void)retarget_to_target();
}

bool DSPBridge::retarget_to_target() noexcept {
    if (mode() != DSPMode::pcm || !fmod_system_) return false;

    const uint32_t handle = target_live_handle();
    if (handle == current_handle_) return false;
